...
export function LeadIntegrationList() {
  ...
  const load = useCallback(async (preserveMessage = false) => {
    ...
    } catch (error) { setMessageIsError(true); setMessage(error instanceof Error ? error.message : "Falha ao carregar leads."); }
  ...
  async function retry(id: string) {
    ...
    setMessage(response.ok ? (result?.message ?? "Lead sincronizado com o DataCrazy.") : (result?.error ?? "Não foi possível tentar novamente."));
    await load(true);
  }
  ...
  return <main className="adminShell"><div className="adminCard">
    ...
    {message && <p className={messageIsError ? "formError" : "adminNotice"} role="status">{message}</p>}
